...
namespace YumBlazor.Repository
{
    public class OrderRepository : IOrderRespository
    {
        ...
        public async Task<IEnumerable<OrderHeader>> GetAllAsync(string? userId = null)
        {
            ...
            {
                return await _db.OrderHeader.Where(x=> x.UserId == userId).ToListAsync();
            }
        ...
        {
            return await _db.OrderHeader.Include(x => x.OrderDetails).FirstOrDefaultAsync(u => u.Id == id) ?? new OrderHeader(); //Handle Null later
        }
        ...
        public async Task<OrderHeader> UpdateStatusAsync(int orderId, string status)
        {
            ...
            return orderHeader ?? new OrderHeader();  //Handle Null later
        }
    }
}
